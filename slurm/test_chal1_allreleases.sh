...
#!/bin/bash
#SBATCH --job-name=test_1_${model_name}
#SBATCH --output=${EEGCHALLENGE_LOGS}/chal_1/all/test_1_${model_name}_%a.out
#SBATCH --error=${EEGCHALLENGE_LOGS}/chal_1/all/test_1_${model_name}_%a.err
#SBATCH --array=1
#SBATCH --time=1:00:00
#SBATCH --partition=gpu,owners,menon,normal
#SBATCH --gres=gpu:1
#SBATCH --mem=40G
...
# Run Challenge 1 with model-specific parameters
python "${EEGCHALLENGE_REPO}/pipelines/evaluate_challenge1.py" \\
    --data_folder \${EEGCHALLENGE_DATA} \\
    --results_folder \${EEGCHALLENGE_RESULTS}/braindecode_all/challenge_1/\${SLURM_ARRAY_TASK_ID} \\
    ...
    --cache_dataset \\
    --load_cached_dataset \\
    --cache_dir \${EEGCHALLENGE_DATA}/braindecode_cache/challenge_1/all \\
    --save_metrics \\
    --save_plots \\
    --save_model \\
    --num_workers 4 \\
...
echo "All jobs submitted successfully for all releases!"
